function x({
    ...
    return {
        ...
        withinDropdownMounted: !1,
        init() {
            let t = this.getTabs(),
                e = new URLSearchParams(window.location.search);
            (u && e.has(r) && t.includes(e.get(r)) && (this.tab = e.get(r)),
                (!this.tab || !t.includes(this.tab)) && (this.tab = t[h - 1]),
                this.$watch('tab', () => {
                    ...
                    ({ message: i, onSuccess: a }) => {
                        a(() => {
                            this.$nextTick(() => {
                                ...
                                let l = this.getTabs();
                                l.includes(this.tab) ||
                                    (this.tab = l[h - 1] ?? this.tab);
                            });
                        });
                    },
                )),
                (this.boundResetHandler = (i) => {
                    i.detail.livewireId !== g ||
                        i.detail.schemaKey !== D ||
                        T ||
                        u ||
                        this.$nextTick(() => {
                            this.tab = this.getTabs()[h - 1] ?? this.tab;
                        });
                }),
        ...
        calculateAvailableWidth(t) {
            let e = window.getComputedStyle(t);
            return (
                Math.floor(t.clientWidth) -
                Math.ceil(parseFloat(e.paddingLeft)) * 2
            );
        },
        calculateContainerGap(t) {
            let e = window.getComputedStyle(t);
            return Math.ceil(parseFloat(e.columnGap));
        },
        calculateDropdownIconWidth(t) {
            let e = t.querySelector('.fi-icon');
            return Math.ceil(e.clientWidth);
        },
        calculateTabItemGap(t) {
            let e = window.getComputedStyle(t);
            return Math.ceil(parseFloat(e.columnGap) || 8);
        },
        calculateTabItemPadding(t) {
            let e = window.getComputedStyle(t);
            return (
                Math.ceil(parseFloat(e.paddingLeft)) +
                Math.ceil(parseFloat(e.paddingRight))
            );
        },
        findOverflowIndex(t, e, i, a, l, b) {
            let p = t.map((n) => Math.ceil(n.clientWidth)),
                w = t.map((n) => {
                    let d = n.querySelector('.fi-tabs-item-label'),
                        s = n.querySelector('.fi-badge'),
                        o = Math.ceil(d.clientWidth),
            ...
            for (let n = 0; n < t.length; n++) {
                let d = p.slice(0, n + 1).reduce((f, I) => f + I, 0),
                    s = n * i,
                    o = w.slice(n + 1),
                    c = o.length > 0,
                    v = c ? Math.max(...o.map((f) => f.total)) : 0,
                    y = c ? l + v + a + b + i : 0;
                if (d + s + y > e) return n;
            }
            return -1;
        ...
        get isDropdownButtonVisible() {
            return this.withinDropdownMounted
                ? this.withinDropdownIndex === null
                    ? !1
                    : this.getTabs().findIndex((e) => e === this.tab) <
        ...
        async updateTabsWithinDropdown() {
            ...
            let l = this.calculateAvailableWidth(t),
                b = this.calculateContainerGap(t),
                p = this.calculateDropdownIconWidth(e),
                w = this.calculateTabItemGap(i[0]),
                n = this.calculateTabItemPadding(i[0]),
                d = this.findOverflowIndex(i, l, b, w, n, p);
